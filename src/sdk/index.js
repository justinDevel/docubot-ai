...
class DocuBotWidget {
  ...
  init(config) {
    ...
    if (!this.config.apiKey || !this.config.documentId) {
      console.error('DocuBot: apiKey and documentId are required');
      return;
    }
  ...
  createWidget() {
    ...
    this.container.style.setProperty('--docubot-primary', this.config.primaryColor);
    this.container.style.setProperty('--docubot-accent', this.config.accentColor);
  ...
  async loadInitialMessage() {
    try {
      ...
      const docResponse = await this.callAPI(`/documents/${this.config.documentId}`, 'GET');
      const documentName = docResponse.document?.fileName || 'your document';
      
      const welcomeMessage = {
        id: 'welcome',
        type: 'bot',
        content: `Hi! I'm your AI assistant for "${documentName}". I can help you find information, answer questions, and provide summaries. What would you like to know?`,
        timestamp: new Date()
      };
  ...
  async sendMessage() {
    ...
    try {
      // Call real API
      const response = await this.callAPI('/chat', 'POST', {
